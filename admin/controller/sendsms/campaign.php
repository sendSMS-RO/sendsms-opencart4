...
class Campaign extends \Opencart\System\Engine\Controller {
    ...
    public function index()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            $this->response->redirect($this->url->link('extension/sendsms/sendsms/campaign.filter', 'user_token=' . $this->session->data['user_token']. $url, true));
        }
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/sendsms/sendsms/campaign', 'user_token=' . $this->session->data['user_token'], true)
        );
        ...
        $data['action'] = $this->url->link('extension/sendsms/sendsms/campaign.index', 'user_token=' . $this->session->data['user_token'], true);
        ...
        $this->load->model('extension/sendsms/module/sendsms/orders');
        $data['counties'] = $this->model_extension_sendsms_module_sendsms_orders->getCounties();
        ...
        $this->response->setOutput($this->load->view('extension/sendsms/sendsms/campaign', $data));
    }

    protected function validate()
    {
        if (!$this->user->hasPermission('access', 'extension/sendsms/sendsms/campaign')) {
    ...
    public function filter()
    {
        ...
        $data['button_cancel'] = $this->language->get('button_cancel');
        $data['save'] = $this->url->link('extension/sendsms/sendsms/campaign.save', 'user_token=' . $this->session->data['user_token']);
        $data['cancel'] = $this->url->link('extension/sendsms/sendsms/campaign', 'user_token=' . $this->session->data['user_token'], true);
        ...
        $this->load->model('extension/sendsms/module/sendsms/orders');
        $data['phones'] = $this->model_extension_sendsms_module_sendsms_orders->getPhoneNumbers($filters);
        ...
        $this->response->setOutput($this->load->view('extension/sendsms/sendsms/filtered', $data));
    }
    ...
    public function save(): void
    {
		...
		if (!$json) {
            ...
            foreach ($this->request->post['module_sendsms_phones'] as $phone) {
                $this->model_extension_sendsms_module_sendsms_send->send_sms($phone, $this->request->post['module_sendsms_message'], 'campaign');
            }

            $json['success'] = $this->language->get('text_success_campaign_send');
		}
