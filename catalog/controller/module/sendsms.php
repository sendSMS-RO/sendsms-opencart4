<?php
namespace Opencart\Catalog\Controller\Extension\Sendsms\Module;

class Sendsms extends \Opencart\System\Engine\Controller
{
    public function statusChange(string $route, array $data): void
    {
        ...
        if ($oldOrderStatusId != $orderStatusId && $enabled == '1') {
            ...
            # send sms
            $this->sendMessage($order['telephone'], $message, 'order', $shortUrl);
        }
    }

    public function sendMessage($phone, $message, $type = 'order', $shortUrl = false)
    {
        $this->load->model('setting/setting');
        ...
        if (!empty($phone) && !empty($username) && !empty($password) && !empty(trim($message))) {
            ...
            # add to history
            $this->load->model('extension/sendsms/module/history');
            $this->model_extension_sendsms_module_history->addHistory(
